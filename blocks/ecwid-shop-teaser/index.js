...
function getPreviewItems(limit) {
	...
	const count = Number.isNaN(parsed) ? 3 : Math.min(Math.max(parsed, 1), 24);
...
function getPresentationStyle(attributes, editorSettings = {}) {
	...
	const titleColor =
		attributes.titleColor || attributes.descriptionColor || legacyTextColor;
...
function DimensionsToolsPanel(props) {
	...
	const wrapperPaddingLabel = __('Wrapper padding', 'ran-ecwid-shop-teaser');
